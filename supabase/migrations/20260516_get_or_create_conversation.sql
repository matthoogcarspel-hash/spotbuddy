...
-- Werkt voor: spot, group, en dm conversations
...
BEGIN
  IF p_type = 'dm' THEN
    -- Zoek bestaand DM gesprek in beide richtingen
    ...
    IF conv_id IS NULL THEN
      ...
      VALUES ('dm', my_id, p_other_user_id)
      RETURNING id INTO conv_id;
    END IF;

  ELSIF p_type = 'group' THEN
    -- Zoek bestaand group gesprek
    SELECT id INTO conv_id
    FROM conversations
    WHERE type = 'group'
      AND spot_name = p_spot_name
      AND session_day = p_session_day::date
      AND group_key = p_group_key
    LIMIT 1;

    IF conv_id IS NULL THEN
      INSERT INTO conversations (type, spot_name, session_day, group_key)
      VALUES ('group', p_spot_name, p_session_day::date, p_group_key)
      RETURNING id INTO conv_id;
    END IF;

  ELSIF p_type = 'spot' THEN
    -- Zoek bestaand spot gesprek
    SELECT id INTO conv_id
    FROM conversations
    WHERE type = 'spot'
      AND spot_name = p_spot_name
    ...
    IF conv_id IS NULL THEN
      ...
      VALUES ('spot', p_spot_name, p_session_day::date)
      RETURNING id INTO conv_id;
    END IF;
  END IF;
