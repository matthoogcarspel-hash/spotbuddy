...
const quickCheckInEndMinutes = 22 * 60;
...
export default Sentry.wrap(function App() {
  ...
  const [pendingJoinReqs, setPendingJoinReqs] = useState<Array<{ id: string; nominee: { id: string; display_name: string; avatar_url: string | null }; introduced_by: { display_name: string } }>>([]);
  ...
  type ReplyMeta = { reply_to_id?: string; reply_to_text?: string | null; reply_to_name?: string };
  const sendSpotMessageInChatTab = async (chatKey: string, mediaUrl: string | null = null, replyMeta: ReplyMeta = {}) => {
    ...
    if (!convId) return;
    const { error } = await supabase.from('messages').insert({ user_id: senderId, text: text || null, spot_name: spotName, session_day: day, conversation_id: convId, created_at: new Date().toISOString(), media_url: mediaUrl ?? null, media_type: mediaUrl ? 'image' : null, ...replyMeta });
  ...
  const sendSessionMessageInChatTab = async (groupKey: string, spotName: string, sessionDay: string, mediaUrl: string | null = null, replyMeta: ReplyMeta = {}) => {
    ...
    if (!convId) return;
    const { data: inserted, error } = await supabase.from('messages').insert({ user_id: senderId, text: text || null, spot_name: spotName, session_day: sessionDay, conversation_id: convId, created_at: new Date().toISOString(), media_url: mediaUrl ?? null, media_type: mediaUrl ? 'image' : null, ...replyMeta }).select('id').single();
  ...
  const sendDmMessage = async (conversationId: string, mediaUrl: string | null = null, replyMeta: ReplyMeta = {}) => {
  ...
  const sendPersistentGroupMessage = async (groupId: string, convId: string, mediaUrl: string | null = null, replyMeta: ReplyMeta = {}) => {
  ...
  if (showChat) {
    ...
    const handleOpenSend = async () => {
      ...
      setReplyingTo(null);
      const replyMeta = currentReply ? { reply_to_id: currentReply.id, reply_to_text: currentReply.text ?? null, reply_to_name: currentReply.display_name } : {};
      if (expandedChatSpot) void sendSpotMessageInChatTab(expandedChatSpot, mediaUrl, replyMeta);
      else if (expandedChatSession) {
        const d = chatSessionMessages[expandedChatSession];
        const spotName = d?.spotName ?? openConvName;
        const sessionDay = d?.sessionDay ?? getTodayLocalDateKey();
        void sendSessionMessageInChatTab(expandedChatSession, spotName, sessionDay, mediaUrl, replyMeta);
      }
        ...
        if (grp?.conversationId) void sendPersistentGroupMessage(expandedPersistentGroupId, grp.conversationId, mediaUrl, replyMeta);
      }
      else if (expandedDmId) void sendDmMessage(expandedDmId, mediaUrl, replyMeta);
    };
    ...
    const chatContent = (
      ...
      {showNominateModal && (() => {
        ...
        const closeModal = () => { setShowNominateModal(null); setNominateSearchQuery(''); setNominateSelectedUserId(null); setNominateSearchResults([]); setAddBuddySelectedIds([]); setPendingJoinReqs([]); };
        ...
        return (
          <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: theme.bg, zIndex: 300, flex: 1 }}>
            ...
            <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled">
              ...
              {isAdmin && pendingJoinReqs.length > 0 && nominateSearchQuery.trim().length === 0 && (
                <View>
                  ...
                  {pendingJoinReqs.map((req) => (
                    <View key={req.id} style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, gap: 14, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.06)' }}>
                      ...
                      <Pressable onPress={async () => { await resolveJoinRequest(req.id, true); setPendingJoinReqs((p) => p.filter((r) => r.id !== req.id)); }} style={{ backgroundColor: '#123868', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 6, marginRight: 6 }}>
                        <Text style={{ color: theme.text, fontSize: 13, fontWeight: '900' }}>Accept</Text>
                      </Pressable>
                      <Pressable onPress={async () => { await resolveJoinRequest(req.id, false); setPendingJoinReqs((p) => p.filter((r) => r.id !== req.id)); }} style={{ backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 6 }}>
            ...
            return (
              <View style={{ position: 'absolute', top: 88, left: 0, right: 0, bottom: 0, backgroundColor: theme.bg, zIndex: 300, flex: 1 }}>
                ...
                <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled">
                  {isAdmin && nominateSearchResults.map((u) => {
                    const sel = addBuddySelectedIds.includes(u.id);
                    return <Pressable key={u.id} onPress={() => setAddBuddySelectedIds((prev) => sel ? prev.filter((id) => id !== u.id) : [...prev, u.id])} style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, gap: 14, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.06)' }}>
                      <Avatar uri={u.avatar_url} size={44} name={u.display_name} />
                      <Text style={{ flex: 1, color: theme.text, fontSize: 15, fontWeight: '700' }}>{u.display_name}</Text>
                      <View style={{ width: 22, height: 22, borderRadius: 11, borderWidth: 2, borderColor: sel ? theme.primary : 'rgba(255,255,255,0.2)', backgroundColor: sel ? theme.primary : 'transparent', alignItems: 'center', justifyContent: 'center' }}>{sel && <Ionicons name="checkmark" size={13} color={theme.bg} />}</View>
                    </Pressable>;
                  })}
                  {nominateSearchQuery.trim().length === 0 && buddyList.length > 0 && (
                    <View>
                  ...
                  {nominateSearchQuery.trim().length === 0 && buddyList.length === 0 && <Text style={{ color: theme.textMuted, fontSize: 14, textAlign: 'center', marginTop: 40 }}>All your buddies are already in this group</Text>}
                  {isAdmin && pendingJoinReqs.length > 0 && nominateSearchQuery.trim().length === 0 && (
                    <View>
                      <Text style={{ color: theme.textMuted, fontSize: 12, fontWeight: '700', paddingHorizontal: 16, paddingTop: 16, paddingBottom: 8 }}>PENDING REQUESTS</Text>
                      {pendingJoinReqs.map((req) => (
                        <View key={req.id} style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 16, paddingVertical: 12, gap: 14, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.06)' }}>
                          <Avatar uri={req.nominee.avatar_url} size={44} name={req.nominee.display_name} />
                          <View style={{ flex: 1 }}><Text style={{ color: theme.text, fontSize: 15, fontWeight: '700' }}>{req.nominee.display_name}</Text><Text style={{ color: theme.textMuted, fontSize: 12 }}>Suggested by {req.introduced_by.display_name}</Text></View>
                          <Pressable onPress={async () => { await resolveJoinRequest(req.id, true); setPendingJoinReqs((p) => p.filter((r) => r.id !== req.id)); }} style={{ backgroundColor: '#123868', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 6, marginRight: 6 }}><Text style={{ color: theme.text, fontSize: 13, fontWeight: '900' }}>Accept</Text></Pressable>
                          <Pressable onPress={async () => { await resolveJoinRequest(req.id, false); setPendingJoinReqs((p) => p.filter((r) => r.id !== req.id)); }} style={{ backgroundColor: 'rgba(255,255,255,0.08)', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 6 }}><Text style={{ color: theme.textMuted, fontSize: 13, fontWeight: '700' }}>Deny</Text></Pressable>
                        </View>
                      ))}
                    </View>
                  )}
